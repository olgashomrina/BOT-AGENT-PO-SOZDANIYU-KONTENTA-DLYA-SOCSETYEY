...
STRINGS = {
    ...
    "double_delete_confirm_text": (
        "Удалю кружки, расшифровки, голос, лицо и образы. Отменить это будет "
        "нельзя — двойника придётся собирать заново."
    ),
    ...
    "speech_delivered_not_recorded": (
        "Кружок отправлен, но не сохранился — опубликовать его через бота "
        "больше нельзя. Текст и озвучка сохранены, повтор будет стоить "
        "только съёмку."
    ),
